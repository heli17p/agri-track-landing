...
export const Dashboard: React.FC<Props> = ({ onNavigate }) => {
  ...
  useEffect(() => { 
      load(); 
      const unsubscribeSync = dbService.onSyncComplete(() => { load(); });
      const unsubscribeDb = dbService.onDatabaseChange(() => { load(); });
  ...
  const yearStats = useMemo(() => {
      ...
      stats.totalActivities = yearActivities.length;
      ...
      yearActivities.forEach(act => {
          if (act.type === ActivityType.FERTILIZATION) {
              ...
              if (notes.includes(HarvestType.HAY)) stats.hayCount += act.amount || 0;
              else if (notes.includes(HarvestType.STRAW)) stats.strawCount += act.amount || 0;
  ...
  const generatePDF = () => {
    const doc = new jsPDF();
    doc.setFontSize(18); doc.text(`Tätigkeitsnachweis ${filterYear}`, 14, 20);
    doc.setFontSize(10);
    ...
    doc.setFontSize(12); doc.text("Jahreszusammenfassung:", 14, 55);
    doc.setFontSize(9);
    let yPos = 62;
    ...
    if(yearStats.strawCount > 0) { doc.text(`- Strohballen (Zukauf): ${yearStats.strawCount} Stk`, 14, yPos); yPos += 5; }

    if (filterType !== 'All') { doc.text(`Filter: ${filterType}`, 14, yPos + 5); yPos += 5; }
    ...
    const exportData = filteredActivities.map(act => {
            ...
            let involvedFieldNames = involvedFieldsList.map(f => {
                let suffix = '';
                if (act.fieldDistribution && act.fieldDistribution[f.id]) suffix = ` (${act.fieldDistribution[f.id]} ${act.unit})`;
                return f.usage ? `${f.name} (${f.usage})${suffix}` : `${f.name}${suffix}`;
            }).join(', ');
            
            if (involvedFieldNames === '' && act.type === ActivityType.HARVEST && act.notes?.includes(HarvestType.STRAW)) involvedFieldNames = 'Zukauf (Keine Fläche)';
            else if (!involvedFieldNames && act.fieldIds.length > 0) involvedFieldNames = '(Gelöschte Felder)';

            let typeLabel: string = act.type;
            if (act.type === ActivityType.HARVEST) {
                if (act.notes && act.notes.includes(HarvestType.HAY)) typeLabel = "Ernte - Heu";
                else if (act.notes && act.notes.includes(HarvestType.STRAW)) typeLabel = "Ernte - Stroh";
                else typeLabel = "Ernte - Silage";
  ...
  const getActivityStyle = (act: ActivityRecord) => {
    ...
    if (act.type === ActivityType.HARVEST) {
        Icon = Wheat;
        const notes = act.notes || '';
        if (notes.includes(HarvestType.HAY)) { label = 'Heu Ernte'; colorClass = 'border-yellow-400'; bgClass = 'bg-yellow-50'; textClass = 'text-yellow-800'; }
        else if (notes.includes(HarvestType.STRAW)) { label = 'Ernte (Stroh)'; colorClass = 'border-yellow-600'; bgClass = 'bg-yellow-50'; textClass = 'text-yellow-900'; Icon = ShoppingBag; }
        ...
        if (act.tillageType === TillageType.MULCH) { colorClass = 'border-indigo-500'; bgClass = 'bg-indigo-50'; textClass = 'text-indigo-800'; }
        else if (act.tillageType === TillageType.WEEDER) { colorClass = 'border-sky-400'; bgClass = 'bg-sky-50'; textClass = 'text-sky-800'; }
        else if (act.tillageType === TillageType.RESEEDING) { colorClass = 'border-teal-500'; bgClass = 'bg-teal-50'; textClass = 'text-teal-800'; }
    }
    ...
    <div className="bg-slate-50 h-full overflow-y-auto pb-24 scroll-smooth relative">
       {showSyncSuccess && (
           <div className="absolute top-16 left-1/2 transform -translate-x-1/2 bg-green-600/90 backdrop-blur text-white px-4 py-2 rounded-full shadow-xl z-50 flex items-center animate-in fade-in slide-in-from-top-4 duration-300">
               <CheckCircle size={18} className="mr-2"/> <span className="font-bold text-sm">Sync OK</span>
           </div>
       )}
       ...
       <div className="p-4 space-y-6">
           <div className="flex justify-between items-start">
               <div>
                   <div className="flex items-center space-x-3 mb-2">
                       <img src={getAppIcon(settings.appIcon || 'standard')} alt="Logo" className="w-12 h-12 rounded-xl shadow-sm border border-slate-200 object-contain bg-white" />
                       <div><h1 className="text-xl font-bold text-slate-800 leading-none">AgriTrack Austria</h1><p className="text-slate-500 text-xs font-medium mt-1">{profile?.operatorName || 'Kein Betrieb'}</p></div>
                   </div>
                   {lastSyncTime && <p className="text-[10px] text-slate-400 mt-2 flex items-center"><CheckCircle size={10} className="mr-1"/> Sync: {new Date(lastSyncTime).toLocaleDateString('de-AT')} {new Date(lastSyncTime).toLocaleTimeString('de-AT', {hour: '2-digit', minute:'2-digit'})}</p>}
               </div>
               <button onClick={handleSync} className="p-2 bg-blue-100 text-blue-700 rounded-full hover:bg-blue-200 shadow-sm mt-1">{loading ? <RefreshCw className="animate-spin"/> : <Cloud />}</button>
           </div>
           
           <div className="flex items-center justify-between border-b border-slate-200 pb-2">
               <h2 className="text-lg font-bold text-slate-700">Übersicht</h2>
               <div className="relative">
                   <select value={filterYear} onChange={(e) => setFilterYear(parseInt(e.target.value))} className="pl-8 pr-3 py-1 bg-slate-100 text-slate-700 rounded-lg text-sm font-bold border-none outline-none appearance-none">
           ...
           <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-white p-4 rounded-xl shadow-sm">
                    <div className="text-slate-500 text-xs uppercase font-bold">Einträge ({filterYear})</div>
                    <div className="text-2xl font-bold text-blue-600">{filteredActivities.length}</div>
                </div>
           </div>

           {/* --- STATS SUMMARY (ALWAYS VISIBLE NOW) --- */}
           <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-100 space-y-4">
               ...
               {yearStats.totalActivities === 0 ? (
                   <div className="text-center py-4 text-slate-400 text-sm italic bg-slate-50 rounded-lg border border-slate-100 border-dashed">
                       Keine Daten für das Jahr {filterYear}.<br/>Wählen Sie ein anderes Jahr oder erfassen Sie eine Tätigkeit.
                   </div>
               ) : (
                   <>
                       ...
                       {(yearStats.silageCount > 0 || yearStats.hayCount > 0 || yearStats.strawCount > 0) && (
                           <div className="space-y-3 pt-2">
                               ...
                               <div className="grid grid-cols-2 gap-3">
                                   {yearStats.silageCount > 0 && <div className="bg-lime-50 p-3 rounded-lg border border-lime-100"><div className="flex items-center text-lime-800 text-xs font-bold mb-1"><Wheat size={12} className="mr-1"/> Silage</div><div className="text-lg font-bold text-lime-900">{yearStats.silageCount} <span className="text-xs">Ballen</span></div></div>}
                                   {yearStats.hayCount > 0 && <div className="bg-yellow-50 p-3 rounded-lg border border-yellow-100"><div className="flex items-center text-yellow-800 text-xs font-bold mb-1"><Wheat size={12} className="mr-1"/> Heu</div><div className="text-lg font-bold text-yellow-900">{yearStats.hayCount} <span className="text-xs">Ballen</span></div></div>}
                                   {yearStats.strawCount > 0 && <div className="bg-yellow-100 p-3 rounded-lg border border-yellow-200"><div className="flex items-center text-yellow-900 text-xs font-bold mb-1"><ShoppingBag size={12} className="mr-1"/> Stroh</div><div className="text-lg font-bold text-yellow-900">{yearStats.strawCount} <span className="text-xs">Ballen</span></div></div>}
                               </div>
                           </div>
                       )}
                       {(yearStats.harrowHa > 0 || yearStats.mulchHa > 0 || yearStats.weederHa > 0 || yearStats.reseedHa > 0) && (
                           <div className="space-y-2 pt-2">
                               <div className="text-xs font-bold text-slate-400 uppercase border-b border-slate-100 pb-1">Bodenbearbeitung (Fläche)</div>
                               <div className="space-y-1 text-sm">
                                   {yearStats.harrowHa > 0 && <div className="flex justify-between items-center bg-blue-50 px-3 py-1.5 rounded text-blue-900"><span className="flex items-center"><Sprout size={12} className="mr-2"/> Wiesenegge</span><span className="font-bold">{yearStats.harrowHa.toFixed(2)} ha</span></div>}
                                   {yearStats.mulchHa > 0 && <div className="flex justify-between items-center bg-indigo-50 px-3 py-1.5 rounded text-indigo-900"><span className="flex items-center"><Hammer size={12} className="mr-2"/> Schlegler</span><span className="font-bold">{yearStats.mulchHa.toFixed(2)} ha</span></div>}
                                   {yearStats.weederHa > 0 && <div className="flex justify-between items-center bg-sky-50 px-3 py-1.5 rounded text-sky-900"><span className="flex items-center"><Hammer size={12} className="mr-2"/> Striegel</span><span className="font-bold">{yearStats.weederHa.toFixed(2)} ha</span></div>}
                                   {yearStats.reseedHa > 0 && <div className="flex justify-between items-center bg-teal-50 px-3 py-1.5 rounded text-teal-900"><span className="flex items-center"><Sprout size={12} className="mr-2"/> Nachsaat</span><span className="font-bold">{yearStats.reseedHa.toFixed(2)} ha</span></div>}
                               </div>
                           </div>
                       )}
                   </>
           ...
           {storages.length > 0 && (
             <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-100 space-y-3">
               <div className="flex justify-between items-center">
                   <h3 className="font-bold text-lg text-slate-800">Lagerstände (Aktuell)</h3>
                   <span className="text-xs text-slate-400 font-medium cursor-pointer hover:text-blue-500" onClick={() => onNavigate('settings')}>Verwalten</span>
               </div>
               <div className="grid grid-cols-1 gap-3">
                   {storages.map(s => {
                       ...
                       const Icon = s.type === FertilizerType.SLURRY ? Droplets : Layers;
                       ...
                       return (
                           <div key={s.id} onClick={() => setSelectedStorage(s)} className="border border-slate-100 rounded-lg p-3 cursor-pointer hover:bg-slate-50 transition-colors">
                               <div className="flex justify-between items-center mb-1">
                                   <div className="flex items-center space-x-2"><Icon size={14} className="text-slate-400"/><span className="font-bold text-sm text-slate-700">{s.name}</span></div>
                                   <div className={`text-xs font-bold ${textColor}`}>{percent.toFixed(0)}%{isFull && <AlertTriangle size={12} className="inline ml-1 mb-0.5"/>}</div>
                               </div>
           ...
           <div className="bg-white p-3 rounded-xl shadow-sm border border-slate-100 space-y-3">
               <div className="flex justify-between items-center"><h3 className="font-bold text-lg text-slate-800">Tätigkeiten Liste</h3></div>
               <div className="flex space-x-2 overflow-x-auto pb-1 hide-scrollbar">
                    <button onClick={() => setSortOrder(prev => prev === 'desc' ? 'asc' : 'desc')} className="p-2 bg-slate-100 text-slate-600 rounded-lg hover:bg-slate-200" title={sortOrder === 'desc' ? "Neueste zuerst" : "Älteste zuerst"}>{sortOrder === 'desc' ? <ArrowDown size={18}/> : <ArrowUp size={18}/>}</button>
                                ...
                                <div onClick={() => setSelectedActivity(act)} className={`p-3 rounded-lg shadow-sm border-l-4 cursor-pointer hover:bg-white transition-all ${style.colorClass} ${style.bgClass}`}>
                                    ...
                                    <div className="space-y-1 mb-2 pl-6 border-l border-black/5">
                                        {involvedFields.length > 0 ? involvedFields.slice(0, 3).map(f => {
                                            ...
                                            if (act.type === ActivityType.HARVEST && act.amount && totalArea > 0) { const share = (f.areaHa / totalArea) * act.amount; detailText = `${share.toFixed(1)} Stk`; }
                                            if (act.fieldDistribution && act.fieldDistribution[f.id]) { detailText = `${act.fieldDistribution[f.id]} ${act.unit}`; }
                                        ...
                                        }) : (style.label.includes('Stroh') ? <span className="text-xs italic text-slate-500">Zukauf (Keine Felder)</span> : <span className="text-xs italic text-slate-400">Gelöschte Felder</span>)}
                                    ...
                                    <div className="border-t border-slate-200/50 pt-2 flex justify-end items-center">
                                        ...
                                        <div className="text-right">
                                            ...
                                            {act.loadCount && act.loadCount > 0 && <div className="text-xs text-slate-500 font-medium">({act.loadCount} Fuhren)</div>}
                                        </div>
                                    </div>
                                </div>
                            </React.Fragment>
