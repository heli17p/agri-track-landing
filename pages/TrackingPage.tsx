...
export const TrackingPage: React.FC<Props> = ({ onTrackingStateChange, onMinimize, onNavigate }) => {
  ...
  const storageBreakdown = useMemo(() => {
      ...
      // Clone the map to avoid mutating ref directly for render purposes
      const counts = new Map(storageDeductionsRef.current);
      let totalPlusActive = loads;
      
      // LOGIC UPDATE:
      // Only add +1 if we have a valid source ID (Fixes "0 at start" issue)
      // AND if we are NOT in LOADING state (Fixes "Glitch during timer").
      // While timer (IDLE) runs, we still have the OLD source active, so count remains stable.
      // Once LOADING starts, we are resetting, so we stop counting the old one.
