...
// Zoomt die Karte automatisch so, dass Spur und Felder sichtbar sind
...
const getStorageColor = (storageId: string | undefined, allStorages: StorageLocation[]) => {
    if (!storageId) return '#3b82f6'; // Blau für reinen Transit
...
export const ActivityDetailView: React.FC<Props> = ({ activity, onClose, onUpdate }) => {
  ...
  const getHeaderStyle = () => {
    if (activity.type === ActivityType.HARVEST) {
        ...
        if (notes.includes(HarvestType.HAY)) return { bg: 'bg-yellow-500', title: 'Heu Ernte', Icon: Wheat };
        if (notes.includes(HarvestType.STRAW)) return { bg: 'bg-yellow-600', title: 'Stroh Ernte', Icon: ShoppingBag };
    ...
    return { bg: 'bg-blue-600', title: activity.tillageType || 'Bodenbearbeitung', Icon: Hammer };
  };
  ...
  // Breitenberechnung für die Spur
  let widthMeters = settings.spreadWidth || 12;
  if (activity.type === ActivityType.FERTILIZATION) {
      widthMeters = activity.fertilizerType === FertilizerType.MANURE ? (settings.manureSpreadWidth || 10) : (settings.slurrySpreadWidth || 12);
  }
  const trackWeight = Math.max(8, widthMeters * 1.2);
      ...
      <div className="relative w-full max-w-md bg-white h-full shadow-2xl flex flex-col animate-in slide-in-from-right duration-300" onClick={e => e.stopPropagation()}>
        ...
        <div className={`p-4 ${headerStyle.bg} text-white flex justify-between items-center shrink-0`}>
             <div className="flex items-center space-x-3">
                 <div className="p-2 bg-white/20 rounded-full"><headerStyle.Icon size={24} /></div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-6 bg-slate-50">
            ...
            {((activity.trackPoints && activity.trackPoints.length > 0) || relevantFields.length > 0) && (
                <div className="h-64 w-full rounded-2xl overflow-hidden border border-slate-300 relative shadow-inner bg-slate-200">
                    <MapContainer center={[47.5, 14.5]} zoom={13} style={{ height: '100%', width: '100%' }} zoomControl={false}>
                        ...
                        {relevantFields.map(f => (
                            <Polygon key={f.id} positions={f.boundary.map(p => [p.lat, p.lng])} pathOptions={{ color: f.type === 'Acker' ? '#92400E' : '#15803D', fillOpacity: 0.1, weight: 1 }} />
                        ))}

                        {trackSegments.map((seg, idx) => {
                             // Fallback: Wenn Punkt keine StorageID hat, nimm die erste aus der Verteilung (für Altdaten)
                             ...
                             return (
                                 <React.Fragment key={idx}>
                                     <Polyline positions={seg.points} pathOptions={{ 
                                         color: color, 
                                         weight: seg.isSpreading ? trackWeight : 3, 
                                         opacity: seg.isSpreading ? 0.8 : 0.5,
                                         lineCap: 'round'
                                     }} />
                                     {seg.isSpreading && (
                                         <Polyline positions={seg.points} pathOptions={{ 
            ...
            <div className="bg-white p-5 rounded-2xl shadow-sm border border-slate-200 space-y-4">
                <div>
                    ...
                    <div className="relative">
                        ...
                        <input type="date" value={dateStr} onChange={e => handleChange('date', new Date(e.target.value).toISOString())} className="w-full pl-10 p-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-green-500 outline-none font-bold text-slate-700"/>
                    </div>
                </div>
                <div className="grid grid-cols-2 gap-4">
                    <div>
                        <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Menge ({editedActivity.unit})</label>
                        <input type="number" value={editedActivity.amount || 0} onChange={e => handleChange('amount', parseFloat(e.target.value))} className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-green-500 outline-none font-black text-xl text-slate-800"/>
                    </div>
                    {editedActivity.loadCount !== undefined && (
                        <div>
                            <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Fuhren</label>
                            <input type="number" value={editedActivity.loadCount} readOnly className="w-full p-2.5 bg-slate-100 border border-slate-200 rounded-xl text-slate-500 font-bold"/>
                        </div>
                    )}
                </div>
                <div>
                    <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Notizen</label>
                    <textarea value={editedActivity.notes || ''} onChange={e => handleChange('notes', e.target.value)} rows={3} className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-green-500 outline-none text-sm text-slate-600" placeholder="Anmerkungen..."/>
                </div>
            </div>
                        ...
                        {Object.entries(activity.storageDistribution).map(([sId, amount]) => {
                             ...
                             return (
                                 <div key={sId} className="flex justify-between items-center p-2 bg-slate-50 rounded-lg border border-slate-100">
                                     ...
                                     <span className="font-black text-slate-800">{amount} {activity.unit}</span>
                                 </div>
                             );
                        })}
                    </div>
            ...
            <div className="space-y-3">
                <div className="flex justify-between items-center">
                    <h3 className="font-bold text-slate-700 flex items-center text-sm"><MapPin size={16} className="mr-2 text-green-600"/> Beteiligte Felder</h3>
                    <button onClick={() => setIsEditingFields(!isEditingFields)} className="text-xs font-bold text-blue-600 bg-blue-50 px-2 py-1 rounded-lg border border-blue-100">{isEditingFields ? 'Fertig' : 'Bearbeiten'}</button>
                </div>
                    ...
                    {isEditingFields ? (
                        <div className="max-h-60 overflow-y-auto divide-y divide-slate-100">
                             {allFields.map(f => {
                                 ...
                                 return (
                                     <div key={f.id} onClick={() => toggleField(f.id)} className={`p-3 flex items-center cursor-pointer hover:bg-slate-50 ${isSelected ? 'bg-blue-50/30' : ''}`}>
                                         {isSelected ? <CheckSquare size={20} className="text-blue-600 mr-3"/> : <Square size={20} className="text-slate-300 mr-3"/>}
                                         <div className="flex-1 text-sm font-bold text-slate-700">{f.name} <span className="text-[10px] text-slate-400 font-normal">({f.areaHa} ha)</span></div>
                                     </div>
                                 );
                             })}
                        </div>
                            ...
                            {relevantFields.map(f => (
                                <div key={f.id} className="p-3 flex justify-between items-center">
                                    <div className="flex flex-col">
                                        ...
                                        <span className="text-[10px] text-slate-400 font-medium">{f.usage || 'Keine Nutzung'}</span>
                                    </div>
        ...
        <div className="p-4 bg-white border-t border-slate-200 shrink-0 space-y-2">
            {isDirty && (
                <button onClick={handleSave} className="w-full py-4 bg-green-600 text-white rounded-2xl font-black shadow-lg shadow-green-100 flex items-center justify-center active:scale-95 transition-all"><Save size={20} className="mr-2"/> Änderungen Speichern</button>
            )}
            <button onClick={handleDelete} className={`w-full py-4 rounded-2xl font-black flex items-center justify-center transition-all border-2 active:scale-95 ${deleteStep === 'confirm' ? 'bg-red-600 border-red-600 text-white animate-pulse' : 'bg-white border-red-50 text-red-500'}`}>
                {deleteStep === 'confirm' ? <><AlertTriangle size={20} className="mr-2"/> Unwiderruflich löschen?</> : <><Trash2 size={20} className="mr-2"/> Tätigkeit Löschen</>}
            </button>
        </div>
      </div>
    </div>
