...
import { Clock, Database, Droplets, Truck, Square, Layers, Ban, History, LocateFixed, XCircle, Beaker, Timer, Minimize2, Eye, AlertCircle } from 'lucide-react';
import { StorageLocation, FertilizerType, ActivityType } from '../../types';
import { HistoryFilterMode } from '../../pages/TrackingPage';
...
export const TrackingUI: React.FC<Props> = ({ 
  ...
  // Show background warning once after tracking starts
  useEffect(() => {
    if (trackingState !== 'IDLE') {
        const timer = setTimeout(() => setShowBgWarning(true), 5000);
  ...
  return (
    <>
      ...
      <div className="fixed top-20 left-1/2 transform -translate-x-1/2 z-[1100] w-full max-w-[85%] flex flex-col items-center space-y-3 pointer-events-none">
        ...
        {showBgWarning && trackingState !== 'IDLE' && (
            <div className="bg-slate-900/90 backdrop-blur text-white px-4 py-2 rounded-2xl shadow-2xl flex items-center space-x-3 animate-in fade-in slide-in-from-top-4 pointer-events-auto border border-white/10 max-w-xs">
                <AlertCircle size={20} className="text-amber-400 shrink-0"/>
                <div className="flex-1">
                    <p className="text-[10px] leading-tight font-medium">
                        <strong>Hinweis:</strong> Damit das GPS genau bleibt, sollte AgriTrack im Vordergrund bleiben. Nutze PiP für Videos.
                    </p>
                </div>
                <button onClick={() => setShowBgWarning(false)} className="p-1 hover:bg-white/10 rounded-full"><XCircle size={16}/></button>
            </div>
        )}
        ...
        {detectionCountdown !== null && (
          <div className="bg-blue-600 text-white px-8 py-4 rounded-[2rem] shadow-[0_20px_50px_rgba(37,99,235,0.4)] flex flex-col items-center space-y-1 animate-in zoom-in-95 border-4 border-white pointer-events-auto">
             <div className="flex flex-col items-center">
                <div className="flex items-center space-x-2 mb-1"><Timer size={22} className="animate-spin-slow text-blue-200"/><span className="text-[11px] font-black uppercase tracking-tighter opacity-90">Lagerplatz erkannt</span></div>
                {pendingStorageName && (<span className="text-sm font-bold bg-white/20 px-3 py-0.5 rounded-full mb-2 border border-white/10">{pendingStorageName}</span>)}
             </div>
        ...
        <div className={`bg-white/95 backdrop-blur shadow-2xl border border-slate-300 rounded-full px-5 py-2.5 flex items-center space-x-3 pointer-events-auto transition-all w-fit ${detectionCountdown !== null ? 'opacity-20 scale-75 blur-[1px]' : 'opacity-100'}`}>
          ...
          <div className="flex flex-col">
            <span className="text-[9px] font-bold text-slate-400 uppercase leading-none mb-0.5 tracking-tighter">{activeStorageName ? `Quelle: ${activeStorageName}` : 'Live-Status'}</span>
            <span className="font-bold text-slate-800 text-sm leading-none">{trackingState === 'LOADING' ? 'Laden...' : trackingState === 'SPREADING' ? 'Ausbringung' : 'Transport'}</span>
          </div>
          {trackingState !== 'IDLE' && (
              <div className="ml-2 pl-2 border-l border-slate-200 flex items-center" title="Bildschirm bleibt aktiv">
                  <Eye size={14} className="text-blue-500 animate-pulse" />
              </div>
          )}
        </div>
      </div>
            ...
            <div className="absolute bottom-full left-0 right-0 mb-4 px-4 pointer-events-none z-[1250]">
                <div className="flex flex-wrap justify-center gap-2 max-w-lg mx-auto">
                    {usedStorages.map(([sId, count]) => {
                        ...
                        return (
                            <div key={sId} className="bg-white/90 backdrop-blur-md shadow-lg border border-slate-200 rounded-full pl-1 pr-3 py-1 flex items-center space-x-2 animate-in slide-in-from-bottom-4 duration-300 pointer-events-auto">
                                <div className="w-6 h-6 rounded-full flex items-center justify-center text-white shadow-sm" style={{backgroundColor: color}}><span className="text-[11px] font-black">{count}</span></div>
                                <span className="text-[10px] font-black text-slate-700 tracking-tight uppercase">{s?.name || 'Unbekannt'}</span>
                            </div>
                        );
                    })}
