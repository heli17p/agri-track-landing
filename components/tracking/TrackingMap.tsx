...
const getCursorIcon = (heading: number | null, type: 'tractor' | 'arrow' | 'dot') => {
  ...
  let size = [32, 32];
  if (type === 'tractor') {
    content = `<svg viewBox="0 0 50 50"><rect x="5" y="30" width="12" height="18" rx="2" fill="#1e293b"/><rect x="33" y="30" width="12" height="18" rx="2" fill="#1e293b"/><rect x="8" y="5" width="8" height="10" rx="2" fill="#1e293b"/><rect x="34" y="5" width="8" height="10" rx="2" fill="#1e293b"/><path d="M20 4 L30 4 L30 20 L34 22 L34 40 L16 40 L16 22 L20 20 Z" fill="#16a34a"/><rect x="14" y="24" width="22" height="14" rx="1" fill="#fff" fill-opacity="0.9" stroke="#94a3b8" stroke-width="2"/></svg>`;
    size = [36, 36];
  } else if (type === 'arrow') {
    content = `<svg viewBox="0 0 24 24" fill="#2563eb" stroke="white" stroke-width="2"><path d="M12 2 L22 22 L12 18 L2 22 Z"/></svg>`;
    size = [28, 28];
  } else {
    content = `<div style="width:100%;height:100%;background:#2563eb;border:2px solid #fff;border-radius:50%;"></div>`;
    size = [16, 16];
  }
  return L.divIcon({ className: 'vehicle-cursor', html: `<div style="transform:rotate(${rotation}deg);transition:transform 0.3s ease;width:100%;height:100%;display:flex;align-items:center;justify-content:center;">${content}</div>`, iconSize: [size[0], size[1]], iconAnchor: [size[0] / 2, size[1] / 2] });
};
...
export const TrackingMap: React.FC<Props> = ({ points, fields, storages, currentLocation, mapStyle, followUser, historyTracks, historyMode, vehicleIconType, onZoomChange, zoom, storageRadius, subType }) => {
  ...
  return (
    <MapContainer center={center} zoom={zoom} style={{ height: '100%', width: '100%' }} zoomControl={false}>
      ...
      {currentLocation && <Marker position={center} icon={getCursorIcon(currentLocation.coords.heading, vehicleIconType)} />}
