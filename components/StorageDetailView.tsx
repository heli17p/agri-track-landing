...
import { StorageLocation, ActivityRecord, Field, ActivityType } from '../types';
...
const FitBoundsToContent = ({ tracks, fields }: { tracks: any[], fields: Field[] }) => {
    ...
    useEffect(() => {
        const t = setTimeout(() => {
            ...
            // Add tracks to bounds
            tracks.forEach(segment => {
                segment.forEach((p: any) => bounds.extend([p[0], p[1]]));
            });

            // Add fields to bounds
            fields.forEach(f => {
...
export const StorageDetailView: React.FC<Props> = ({ storage, onClose }) => {
  ...
  const storageColor = getStorageColor(storage.id);
  ...
  useEffect(() => {
      const loadHistory = async () => {
          ...
          const relevant = allActs.filter(a => {
              ...
              // Check 3: Legacy Distribution (Old Data)
  ...
  const trackSegments = useMemo(() => {
      ...
      filteredActivities.forEach(act => {
          ...
          // If the record has NO specific storageIds on points (Old Data),
          // BUT it has a storageDistribution with ONLY ONE storage, and that storage is THIS one...
          // Then we can safely assume ALL spreading points belong to this storage.
          const isLegacySingleSource = 
            ...
            !act.trackPoints.some(p => p.storageId); // Ensure it's actually legacy (no IDs)

          let currentSegment: [number, number][] = [];
          
          act.trackPoints.forEach((p, i) => {
              // Match if: Explicit ID matches OR it's a safe legacy inference
              const isMatch = (p.storageId === storage.id) || (isLegacySingleSource && !p.storageId);

              // Only include points that match AND are spreading
  ...
  // Find fields that were fertilized by this storage (Based on Filtered Activities)
  const affectedFields = useMemo(() => {
      const affectedIds = new Set<string>();
      filteredActivities.forEach(act => {
          // Check precise source mapping
          ...
          // Check tracks intersecting fields (fallback / legacy)
          else if (act.trackPoints) {
               // Simple check: if activity used storage, assume involved fields are affected
               // (This is a simplified view for legacy data)
        ...
        <div className="p-4 bg-slate-800 text-white flex justify-between items-center shrink-0 border-b-4" style={{ borderColor: storageColor }}>
          <div className="flex items-center space-x-3">
            <div className="p-2 bg-white/10 rounded-full">
                {storage.type === 'Gülle' ? <Droplets size={20} className="text-amber-200" /> : <Layers size={20} className="text-orange-200" />}
            </div>
            <div>
                <h2 className="text-xl font-bold">{storage.name}</h2>
                <div className="text-slate-300 text-sm">{storage.type} Lager</div>
            </div>
          </div>
            ...
            <div className="p-6 space-y-6">
                ...
                <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
                    <div className="flex justify-between items-end mb-3">
                        <span className="text-slate-500 font-bold uppercase text-xs tracking-wider">Aktueller Füllstand</span>
                        <span className={`font-bold text-2xl ${isFull ? 'text-red-600' : isWarning ? 'text-amber-600' : 'text-slate-800'}`}>
                    ...
                    <div className="relative w-full bg-slate-200 rounded-full h-6 overflow-hidden shadow-inner">
                        <div 
                            className={`h-full rounded-full transition-all duration-700 ease-out flex items-center justify-end pr-2 ${
                                isFull ? 'bg-red-500' : isWarning ? 'bg-amber-500' : 'bg-green-500'
                            }`} 
                    ...
                    <div className="flex justify-between mt-3 text-sm font-medium text-slate-600">
                        <span>{storage.currentLevel.toFixed(1)} m³</span>
                        <span>{storage.capacity.toFixed(0)} m³ Kapazität</span>
                    </div>
                </div>

                {/* Warning Box */}
                {isFull && (
                    <div className="bg-red-50 p-4 rounded-xl border border-red-200 flex items-start text-red-800">
                        <AlertTriangle className="shrink-0 mr-3 mt-0.5" />
                        <div className="text-sm">
                            <strong>Kritischer Füllstand!</strong><br/>
                            Dieses Lager ist fast voll.
                        </div>
                    </div>
                )}
                ...
                <div className="grid grid-cols-2 gap-4">
                    <div className="p-4 border border-slate-100 rounded-2xl bg-white shadow-sm flex flex-col items-center text-center">
                        ...
                        <div className="text-2xl font-bold text-slate-800">{storage.capacity}</div>
                        <div className="text-xs text-slate-400 uppercase font-bold mt-1">Kapazität (m³)</div>
                    </div>
                    <div className="p-4 border border-slate-100 rounded-2xl bg-white shadow-sm flex flex-col items-center text-center">
                        <div className="bg-green-50 p-2 rounded-full mb-2 text-green-600">
                            <TrendingUp size={20}/>
                        </div>
                        <div className="text-2xl font-bold text-slate-800">{storage.dailyGrowth}</div>
                        <div className="text-xs text-slate-400 uppercase font-bold mt-1">Zuwachs / Tag</div>
                    </div>
                </div>
                ...
                <div className="space-y-3">
                    <div className="flex flex-col space-y-3">
                         <div className="flex justify-between items-center">
                            <h3 className="font-bold text-slate-700 flex items-center">
                                <MapPin className="mr-2" size={18}/> Ausbringungshistorie
                            </h3>
                             <button 
                            ...
                            >
                                 {mapStyle === 'standard' ? 'Satellit' : 'Karte'}
                            </button>
                         </div>
                         ...
                         <div className="relative">
                             <select 
                                ...
                                className="w-full pl-8 pr-2 py-2 bg-white border border-slate-200 rounded-lg text-sm font-medium text-slate-600 appearance-none focus:outline-none focus:ring-2 focus:ring-green-500 shadow-sm"
                             >
                                <option value="all">Alle Jahre (Gesamt)</option>
                    ...
                    <div className="h-64 w-full rounded-xl overflow-hidden border border-slate-300 relative shadow-sm bg-slate-200">
                        <MapContainer key={`map-${filterYear}`} center={[47.5, 14.5]} zoom={13} style={{ height: '100%', width: '100%' }} zoomControl={false}>
                            ...
                            {!loadingHistory && (trackSegments.length > 0 || affectedFields.length > 0) && (
                                <FitBoundsToContent tracks={trackSegments} fields={affectedFields} />
                            )}
                            ...
                            {/* Actual Tracks from this Storage */}
                            ...
                            {trackSegments.length === 0 && affectedFields.length === 0 && !loadingHistory && (
                                <div className="absolute inset-0 flex items-center justify-center bg-white/50 backdrop-blur-sm z-[500]">
                                    <div className="text-center text-slate-500 text-sm font-medium">
                                        Keine Ausbringungsdaten<br/>
                                        {filterYear !== 'all' ? `im Jahr ${filterYear}` : 'gefunden'}.
                                    </div>
                                </div>
                            )}
                        </MapContainer>
                    ...
                    {trackSegments.length > 0 && (
                        <div className="text-[10px] text-slate-500 flex items-center justify-center mt-1">
                            <span className="w-8 h-1 rounded mr-2" style={{backgroundColor: storageColor}}></span>
                            <span>GPS-Spur aus {storage.name}</span>
                        </div>
                    )}
                </div>
