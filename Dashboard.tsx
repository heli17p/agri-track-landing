...
export const Dashboard: React.FC<Props> = ({ onNavigate }) => {
      ...
      yearActivities.forEach(act => {
          ...
          // Harvest
              ...
              if (notes.includes(HarvestType.HAY)) {
                  ...
                  // Default to Silage if neither Hay nor Straw
  ...
  const generatePDF = () => {
    ...
    const exportData = filteredActivities.map(act => {
            ...
            let involvedFieldNames = involvedFieldsList.map(f => {
                let suffix = '';
                // Add specific amount if available
            ...
            // Fallback for deleted fields or Straw (Zukauf)
            if (involvedFieldNames === '' && act.type === ActivityType.HARVEST && act.notes?.includes(HarvestType.STRAW)) {
            ...
            if (act.loadCount) {
                amount += ` (${act.loadCount} Fuhren)`;
            }

            // 5. Notes
            const notes = act.notes || '';

            return [date, involvedFieldNames, typeLabel, amount, notes];
        });
  ...
  const handleSync = async () => {
    try {
        setLoading(true);
        await syncData();
        // localStorage is updated inside syncData now
        await load();
  ...
  // Helper for Activity Styling (Shared logic with TrackingPage)
  const getActivityStyle = (act: ActivityRecord) => {
    ...
    if (act.type === ActivityType.HARVEST) {
        ...
        if (notes.includes(HarvestType.HAY)) {
            ...
            Icon = ShoppingBag; // Different icon for purchase
        } else {
            label = 'Silage Ernte';
       ...
       <div className="p-4 space-y-6">
           ...
           <div className="grid grid-cols-2 gap-4">
                <div 
                  ...
                  className="bg-white p-4 rounded-xl shadow-sm cursor-pointer hover:bg-green-50 transition-colors border border-transparent hover:border-green-100 group"
                >
                    <div className="flex justify-between items-start mb-2">
                        <div className="text-slate-500 text-xs uppercase font-bold">Fläche Ges.</div>
                        <List size={18} className="text-green-600" />
                    </div>
                    <div className="text-2xl font-bold text-green-600 mb-1">{totalArea.toFixed(2)} ha</div>
                    <div className="text-xs text-green-700 font-medium flex items-center group-hover:underline">
                        Felder verwalten <ChevronRight size={14} className="ml-1"/>
                    </div>
                </div>
           ...
           {(yearStats.slurryVol > 0 || yearStats.manureVol > 0 || yearStats.silageCount > 0 || yearStats.hayCount > 0 || yearStats.strawCount > 0 || yearStats.harrowHa > 0) && (
               <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-100 space-y-4">
                   <h3 className="font-bold text-lg text-slate-800 flex items-center">
                       <Calculator className="mr-2 text-slate-500" size={20}/> Jahresstatistik {filterYear}
                   </h3>
                               ...
                               {yearStats.slurryVol > 0 && (
                                   <div className="bg-amber-50 p-3 rounded-lg border border-amber-100">
                                       ...
                                       <div className="text-[10px] text-amber-700 font-medium">
                                           Ø {(totalArea > 0 ? yearStats.slurryVol / totalArea : 0).toFixed(1)} m³/ha
                                       </div>
                                   </div>
                               )}
                               {yearStats.manureVol > 0 && (
                                   <div className="bg-orange-50 p-3 rounded-lg border border-orange-100">
                                       ...
                                       <div className="text-[10px] text-orange-700 font-medium">
                                           Ø {(totalArea > 0 ? yearStats.manureVol / totalArea : 0).toFixed(1)} m³/ha
                                       </div>
                                   </div>
                               )}
                           </div>
                       ...
                       <div className="space-y-2 pt-2">
                           ...
                           <div className="space-y-1 text-sm">
                               {yearStats.harrowHa > 0 && (
                                   <div className="flex justify-between items-center bg-blue-50 px-3 py-1.5 rounded text-blue-900">
                                       <span className="flex items-center"><Sprout size={12} className="mr-2"/> Wiesenegge</span>
                                       <span className="font-bold">{yearStats.harrowHa.toFixed(2)} ha</span>
                                   </div>
                               )}
                               {yearStats.mulchHa > 0 && (
                                   <div className="flex justify-between items-center bg-indigo-50 px-3 py-1.5 rounded text-indigo-900">
                                       <span className="flex items-center"><Hammer size={12} className="mr-2"/> Schlegler</span>
                                       <span className="font-bold">{yearStats.mulchHa.toFixed(2)} ha</span>
                                   </div>
                               )}
                               {yearStats.weederHa > 0 && (
                                   <div className="flex justify-between items-center bg-sky-50 px-3 py-1.5 rounded text-sky-900">
                                       <span className="flex items-center"><Hammer size={12} className="mr-2"/> Striegel</span>
                                       <span className="font-bold">{yearStats.weederHa.toFixed(2)} ha</span>
                                   </div>
                               )}
                               {yearStats.reseedHa > 0 && (
                                   <div className="flex justify-between items-center bg-teal-50 px-3 py-1.5 rounded text-teal-900">
                                       <span className="flex items-center"><Sprout size={12} className="mr-2"/> Nachsaat</span>
                                       <span className="font-bold">{yearStats.reseedHa.toFixed(2)} ha</span>
                                   </div>
                               )}
                           </div>
                       </div>
                           ...
                           >
                               <div className="flex justify-between items-center mb-1">
                                   ...
                                   <div className={`text-xs font-bold ${textColor}`}>
                                       {percent.toFixed(0)}%
                                       {isFull && <AlertTriangle size={12} className="inline ml-1 mb-0.5"/>}
                                   </div>
                               </div>
                               ...
                               <div className="flex justify-between mt-1 text-[10px] text-slate-400 font-medium">
                                   <span>{s.currentLevel.toFixed(1)} m³</span>
                                   <span>{s.capacity.toFixed(0)} m³ Max</span>
                               </div>
                           </div>
                       )
                   })}
           ...
           <div className="bg-white p-3 rounded-xl shadow-sm border border-slate-100 space-y-3">
               ...
               <div className="flex space-x-2 overflow-x-auto pb-1 hide-scrollbar">
                    ...
                    <button 
                       ...
                       className={`px-3 py-1.5 rounded-lg text-xs font-bold whitespace-nowrap transition-colors ${filterType === 'All' ? 'bg-slate-800 text-white' : 'bg-slate-100 text-slate-500'}`}
                    >
                        Alle
                    </button>
                    <button 
                       onClick={() => setFilterType(ActivityType.HARVEST)}
                       className={`px-3 py-1.5 rounded-lg text-xs font-bold whitespace-nowrap transition-colors ${filterType === ActivityType.HARVEST ? 'bg-green-600 text-white' : 'bg-slate-100 text-slate-500'}`}
                    >
                        Ernte
                    </button>
                    <button 
                       onClick={() => setFilterType(ActivityType.FERTILIZATION)}
                       className={`px-3 py-1.5 rounded-lg text-xs font-bold whitespace-nowrap transition-colors ${filterType === ActivityType.FERTILIZATION ? 'bg-amber-600 text-white' : 'bg-slate-100 text-slate-500'}`}
                    >
                        Düngung
                    </button>
                    <button 
                       onClick={() => setFilterType(ActivityType.TILLAGE)}
                       className={`px-3 py-1.5 rounded-lg text-xs font-bold whitespace-nowrap transition-colors ${filterType === ActivityType.TILLAGE ? 'bg-blue-600 text-white' : 'bg-slate-100 text-slate-500'}`}
                    >
                        Boden
                    </button>
               </div>
                                ...
                                >
                                    ...
                                    <div className="space-y-1 mb-2 pl-6 border-l border-black/5">
                                        {involvedFields.length > 0 ? involvedFields.slice(0, 3).map(f => {
                                            ...
                                            // New: Use fieldDistribution if available
                                            if (act.fieldDistribution && act.fieldDistribution[f.id]) {
                                            ...
                                            return (
                                                <div key={f.id} className="flex justify-between text-xs text-slate-600">
                                                    <div className="flex flex-col truncate max-w-[150px]">
                                                        ...
                                                        {f.usage && <span className="text-[10px] text-slate-400">{f.usage}</span>}
                                                    </div>
                                                    <span className="font-medium bg-white/50 px-1 rounded h-fit">{detailText}</span>
                                                </div>
                                            );
                                        }) : (
                                            /* If straw, show Zukauf instead of deleted fields */
                                            style.label.includes('Stroh') ? (
                                                <span className="text-xs italic text-slate-500">Zukauf (Keine Felder)</span>
                                            ) : (
                                                <span className="text-xs italic text-slate-400">Gelöschte Felder</span>
                                            )
                                        )}
                                        {involvedFields.length > 3 && (
                                    ...
                                    <div className="border-t border-slate-200/50 pt-2 flex justify-end items-center">
                                        ...
                                        <div className="text-right">
                                            ...
                                            {/* Show Loads if available */}
                                            {act.loadCount && act.loadCount > 0 && (
           ...
           <button onClick={generatePDF} className="w-full bg-slate-800 text-white py-3 rounded-xl flex items-center justify-center font-semibold shadow-lg hover:bg-slate-900 transition">
              <Download className="mr-2" size={20}/> Bericht {filterYear} Exportieren (PDF)
           </button>
       </div>
