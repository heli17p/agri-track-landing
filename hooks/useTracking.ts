...
export const useTracking = (
  ...
  const handleNewPosition = useCallback((pos: GeolocationPosition) => {
    ...
    setTrackPoints(prev => {
        if (prev.length > 0) {
            ...
            const minMove = isTestModeRef.current ? 0.3 : 0.6; // Größerer Schwellwert für flüssigeres Ziehen
  ...
  const simulateMovement = useCallback((lat: number, lng: number) => {
    ...
    // THROTTLING: Max 10 Updates pro Sekunde (100ms) für die Simulation
    // Das erlaubt Leaflet flüssiges Dragging, während React die Spur seltener nachzeichnet
    if (now - lastSimTimeRef.current < 100) return;
  ...
  const startGPS = async () => {
    ...
    try {
      ...
      lastSimPosRef.current = { lat: initPos.coords.latitude, lng: initPos.coords.longitude }; lastSimTimeRef.current = Date.now();
  ...
  const stopGPS = useCallback(() => { if (watchIdRef.current !== null) navigator.geolocation.clearWatch(watchIdRef.current); setIsTestMode(false); lastSimPosRef.current = null; }, []);

  const handleFinishLogic = async (notes: string) => {
    const finalPoints = [...trackPoints]; stopGPS(); setTrackingState('IDLE');
    let totalAmount = 0;
    ...
    const record: ActivityRecord = {
      id: generateId(), date: new Date().toISOString(), type: activityTypeRef.current,
      ...
      fieldIds: Array.from(new Set(finalPoints.filter(p => p.isSpreading).map(p => {
          const f = fieldsRef.current.find(field => isPointInPolygon(p, field.boundary)); return f ? f.id : null;
      }).filter(id => id !== null) as string[])),
      amount: totalAmount, unit: 'm³', trackPoints: finalPoints, loadCount: totalLoads, storageDistribution: loadCounts,
      year: new Date().getFullYear(), notes: notes + `\nAutomatisch erfasst. Dauer: ${startTime ? Math.round((Date.now()-startTime)/60000) : 0} min`
    };
    await dbService.saveActivity(record); if (Object.keys(loadCounts).length > 0) await dbService.updateStorageLevels(loadCounts);
    return record;
  };
  ...
  return {
    ...
    handleFinishLogic, handleDiscard: () => { stopGPS(); setTrackingState('IDLE'); setTrackPoints([]); }
  };
};
